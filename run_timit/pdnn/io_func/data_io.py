...
# only support utt
def read_dataset_utt(file_path_list, read_opts):
    assert read_opts['utt']==True
    utt_reader=[]
    for pl in file_path_list:
        utt_reader.append(PfileDataRead([pl], read_opts))

    for dr in utt_reader:
        dr.initialize_read(first_time_reading = True)
